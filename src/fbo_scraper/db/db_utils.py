...
class DataAccessLayer:
    ...
    test_db_uris = [
        "postgres://circleci@localhost:5432/smartie-test?sslmode=disable",
...
def posted_date_to_datetime(posted_date_string):
    ...
    return datetime(
        int(parts[0]), int(parts[1]), int(parts[2]), 12
    )  # use noon on the day so that timezone issues don't move it a day earlier/later
...
def insert_data_into_solicitations_table(session, data):
    ...
    for opp in data:
        try:
            ...
            results = session.query(db.Solicitation).filter(
                db.Solicitation.solNum == opp["solnbr"]
            )
            for s in results:
                # make a duplicate
                sol = s
            ...
            original_sol_date = (
                sol.date or datetime.utcnow()
            )  # need this later to see if this is an update or not
            ...
            estar = "yes" if random() < 0.5 else "no"
            sol.category_list = {"value": "yes", "it": "yes", "estar": estar}
            sol.undetermined = False
            sol.title = opp["subject"]
            sol.url = opp["url"]
            sol.contactInfo = opp["emails"]
            agency_alias_query = session.query(db.AgencyAlias).filter(
                db.AgencyAlias.alias == opp["agency"]
            )
            if agency_alias_query.count() > 0:
                ...
                if agency_alias.agency_id:
                    agency = (
                        session.query(db.Agencies)
                        .filter(db.Agencies.id == agency_alias.agency_id)
                        .one()
                    )
                    sol.agency = agency.agency
                    logger.debug(
                        "{} mapped to {} for solnum {}".format(
                            opp["agency"], sol.agency, sol.solNum
                        )
                    )
            else:
                logger.warning(
                    "unable to map agency {} for solnum {}".format(
                        opp["agency"], sol.solNum
                    )
                )
            ...
            if sol_existed_in_db:
                if not sol.history:
                    sol.history = []
                if is_opp_update(
                    existing_date=original_sol_date,
                    posted_date=opp["postedDate"],
                    sol_existed_in_db=sol_existed_in_db,
                ):
                    sol.history.append(
                        {
                            "date": now_datetime_string,
                            "user": "",
            ...
            for doc in attachments:
                ...
                sol_prediction += doc[
                    "prediction"
                ]  # this should be a 0/1 boolean and if any 1 then it's enough to make the total result true
                sol.attachments.append(attachment)
                parse_status_text = (
                    "successfully parsed"
                    if doc["machine_readable"]
                ...
                parseStatus.append(
                    {
                        ...
                        "postedDate": now_datetime_string,
                        "attachment_url": doc["url"],
                    }
                )

            sol.parseStatus = parseStatus
            new_prediction = deepcopy(
                sol.predictions
            )  # make a copy - if you only chagne the props then SQAlchamy won't know the object changed
            if sol_prediction != 0:
            ...
            if sol.noticeData.get("epa_psc_match", False):
                estar = "red" if random() < 0.5 else "green"
            else:
                estar = "Not Applicable"
            new_prediction["estar"] = estar

            new_prediction["history"].append(
                {
                    "date": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ"),
                    "value": new_prediction["value"],
                    "508": new_prediction["value"],
                    "estar": estar,
                }
            )
...
def get_trained_count(session):
    ...
    """
    trained_count = session.query(
        func.sum(case([(db.Attachment.trained is True, 1)], else_=0))
    )
...
def get_validated_untrained_count(session):
    ...
    """
    validated_untrained_count = session.query(
        func.sum(
            case(
                [
                    (
