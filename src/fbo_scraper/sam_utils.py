...
def get_notice_type(notice_type_code):
    ...
    if not sam_notice_type:
        ...
        if notice_type_code not in other_codes:
            logger.warning(
                f"Found an unanticipated notice type with code: {notice_type_code}"
            )
            return
        return
...
def schematize_opp(opp):
    ...
    if (
        ...
        and isinstance(organizationHierarchy, list)
        and len(organizationHierarchy) > 0
    ):
...
def sol_type_filter(opps, types):
    ...
    logger.debug(
        "Total skip stats so far: Skipping {} out of {} due to solicitation type".format(
            _total_skipped, _total_kept + _total_skipped
        )
    )
...
def update_notice_type_if_necessary(sol, sam_data, session):
    ...
    if sol.noticeType != sam_data["type"]:
        logger.info(
            f"Updating the notice type for {sol.solNum} to be {sam_data['type']}"
        )
...
def update_old_solicitations(
    ...
    try:
        ...
        solicitations = (
            session.query(db.Solicitation)
            .filter(db.Solicitation.active is True)
            .filter(db.Solicitation.date > dt.today() - timedelta(age_cutoff))
            .order_by(db.Solicitation.date.desc())
        )
        ...
        for sol in candidate_solicitations:
            ...
            if sol.active != (sam_sol_data["active"].lower() == "yes"):
                ...
                logger.info(
                    f"Updating the active state for {sol.solNum} - setting it inactive"
                )
                stats["updated"] += 1
            else:
                logger.debug(
                    f"Performed check on {sam_sol_data['solicitationNumber']} but no updates were necessary"
                )

            stats["updated"] += update_notice_type_if_necessary(
                sol, sam_sol_data, session
            )

        logger.info(
            "Recheck of old solicitations complete. {} solicitations examined and {} updated ".format(
