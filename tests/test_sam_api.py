...
class SAMAPITestCase(unittest.TestCase):
    ...
    def test_requests_retry_session(self):
        ...
        for opp in ops:
            if not any(
                opp["naicsCode"].startswith(n) for n in sam_utils.naics_code_prefixes
            ):
                self.assertTrue(
                    False,
                    f"Failure - opp was returned that had naics code {opp['naicsCode']}. Only naics codes in {','.join(sam_utils.naics_code_prefixes)} should be returned",
                )
